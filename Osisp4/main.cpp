...
void writeFile(vector<string> sortedData, string outputFilePath);
...
int main()
{
    ...
    string outputFileAdr;
    cin >> outputFileAdr;

    auto words = readFile(inputFileAdr);
    ...
    writeFile(result, outputFileAdr);
    return 0;
}
...
vector<SortTask*> initTasks(vector<string> allWords) {
    ...
    for (int i = 0; i < THREAD_COUNT; ++i) {
        vector<string>* slice = new vector<string>(allWords.begin() + i * wordsPerThread,
            ...
            allWords.end() : allWords.begin() + (i + 1) * wordsPerThread);  //final slice or just next slice
...
vector<string> mergeArrays(vector<string> v1, vector<string> v2) //by couples
{
    int i = 0, j = 0;
    vector<string> res{};
